...
def add_new_extension(name, ext, win, caller=''):
    ...
    try:
        ...
        if name not in ext_database(0, 0) and ext not in ext_database('all_ext', 'e') and caller == '':
            oldall = ext_database('all_ext', 'e')
            oldall.append(ext)
            ...
            messagebox.showinfo('Success!',
                                ...
                                f'Extension : {ext}', parent=win)
        elif caller == 'update':
            if ext not in ext_database('all_ext', 'e'):
                old_ext = ext_database(name, 'e')
                oldall = ext_database('all_ext', 'e')
                old_ext.append(ext)
                oldall.append(ext)
                cur.execute(f'update ext set extension="{",".join(old_ext)}" where name="{name}"')
                cur.execute(f'update ext set extension="{",".join(oldall)}" where name="all_ext"')
                messagebox.showinfo('Success!',
                                    f'A new extension is added Successfully in database. \n\nName : {name}\n\n'
                                    f'Extension : {ext}',
...
def delete_file_type(ftyp, win):
    connection = sqlite3.connect(extensions_DATABASE)
    try:
        allext = ext_database('all_ext', 'e')
        exttodel = ext_database(ftyp, 'e')
        updated_allext = [exts for exts in allext if exts not in exttodel]
        cur = connection.cursor()
        cur.execute(f'delete from ext where name="{ftyp}"')
        cur.execute(f'update ext set extension="{",".join(updated_allext)}" where name="all_ext"')
        connection.commit()
        messagebox.showinfo('Deleted',
                            f'File Type with name : "{ftyp}" and all extensions under its catagory is '
                            f'deleted Successfully.', parent=win)
    finally:
        connection.close()


def del_e_xtensions(name, ext_discard, win):
    extstodel = ext_database(name, 'e')

    allexts = ext_database('all_ext', 'e')
    ...
    if ext_discard in extstodel:
        ...
        try:
            ...
            connection.commit()
            messagebox.showinfo('Deleted', f'Extension with name "{ext_discard}" if deleted successfully from "{name}"',
                                parent=win)
        finally:
            connection.close()
        return 1
...
def ask(nonsorted):
    ...
    askcan.create_text(30, 60,
                       text='If you know the type of extensions they have, '
                            'Please provide us that information so in future we '
                            'will be able to sort them.', anchor=NW)

    pic_holder_can = askglobal_obj.customcan(askcan, 250, 250, 'white', 490, 200)
    pic = PhotoImage(file=nonsorted_graphic)
    pic_holder_can.create_image(0, 0, image=pic, anchor=NW)
    pic_holder_can.create_text(30, 10, text='?', anchor=NW, font='Ebrima 15')
    pic_holder_can.create_line(50, 30, 70, 40)
    ...
    l_box = askobj1.customListBox(list_holder, 50, 13, LEFT)
    l_box.config(yscrollcommand=yscrol.set, justify=CENTER, borderwidth=0)
    yscrol.config(command=l_box.yview)
    ...
    askwin.protocol('WM_DELETE_WINDOW', quit_destroy)
    askwin.mainloop()


def ext_database(extType, ft):
    connection = sqlite3.connect(extensions_DATABASE)
    try:
        if ft == 'e':
            cur = connection.cursor()
...
class AdvanceOperation:
    ...
    def directory_manager(self, sourcepath):
        ...
        prefix = ''
        fetch_x_paths = [sourcepath + f'/{prefix} {f_name}' for f_name in self._extlist]

        for lists in self._temp_l:

            if len(lists) != 0:
                folder = fetch_x_paths[self._directory_counter]
    ...
    def file_processor(self, raw_path):
        ...
        for file_types_ in self._extlist:
            for file_found in _files:
                for _ext in ext_database(file_types_, 'e'):
        ...
        if self.dm_resp[0] == 1:
            remaining_files = os.listdir(raw_path)
            if len(remaining_files) == 0:
                pass
            else:
                for r_files in remaining_files:
...
def file_crash_handler(src, dst):
    ...
    askcan.create_text(100, 15, text='A file with exact same name already exist in this directory.', anchor=NW,
                       font='Ebrima 15')
    askcan.create_line(50, 55, 650, 55)
    ...
    askcan.create_line(100, 200, 600, 200)
    ...
    askcan.create_line(100, 350, 600, 350)
    ...
    chb3 = Checkbutton(askcan, bg='white', text='  Do this for all Files ? ', variable=chb3_var, command=onclick3,
                       state=DISABLED)
    chb3.place(x=490, y=310)

    askcan.create_text(100, 360, text='You can either RENAME the file to avoid clash .', anchor=NW,
                       font='Ebrima 12 bold')

    name_var = StringVar()
    askcan.create_text(100, 400, text='New Name : ', anchor=NW)
    nn_e = Entry(askcan, highlightthickness=1, highlightbackground='black', width=25, textvariable=name_var)
    nn_e.place(x=180, y=398)
    ...
    def proceeding():
        ...
        if chb1_state == 1 and chb3_state != 1:
            ...
            input_list = list(name_e)
            templ = []
            for alpha in input_list:
                if alpha in excluded:
                    errorhandleing('Invalid Charecter',
                                   'Charecters Like     ?, |, /, \\, <, >, ., ",*   are not allowed in name of the file.',
                                   askwin)
            ...
            if templ[0] == 'Abort':
                ...
                rename_resp = renaming_file(src, f'{dst}/{name_e}.{_fileName.split(".")[-1]}')
                if rename_resp == 1:
                    quit_destroy()
                else:
                    errorhandleing('Rename Error',
                                   'A file with the given name is already exist in destination directory.', askwin)
                    quit_destroy()
        else:
            errorhandleing('Action Not Specified', 'YOU MUST SELECT THE ACTION TO BE PERFORMED TO AVOID MALFUNCTION !',
                           askwin)

    askglobal_obj.custombuttons(askcan, 'Proceed', 15, 1, proceeding, 550, 450)

    askwin.protocol('WM_DELETE_WINDOW', lambda: errorhandleing('Action Not Specified',
                                                               'YOU MUST SELECT THE ACTION TO BE PERFORMED TO '
                                                               'AVOID MALFUNCTION !',
                                                               askwin))
    askwin.mainloop()
...
def autoSense_confirmation(root):
    ...
    pic_holder_can = askglobal_obj.customcan(askcan, 250, 250, 'white', 480, 90)
    ...
    def stop_and_exit():
        user_configs.update_val('autosense', 0, '')
        root.destroy()
...
def as_Error():
    ...
    x = (auto_s.winfo_screenwidth() // 2) - (width // 2)
    y = (auto_s.winfo_screenheight() // 2) - (height // 2)

    auto_s.geometry(f'{width}x{height}+{x}+{y}')
    auto_s.attributes('-topmost', 1)
    auto_s.focus()
    auto_s.overrideredirect(1)

    askglobal_obj = Mainframe2()

    auto_s.title('Temporary Error !      AutoSense Directory Not Found')

    auto_s.resizable(0, 0)
    askcan = askglobal_obj.customcan(auto_s, width, height, 'white', 0, 0)
    ...
    b1 = askglobal_obj.custombuttons(askcan, 'OK', 15, 1, quit_destroy, 30, 300)
    b1.config(borderwidth=2, relief='groove', cursor="hand2")
    auto_s.after(10000, quit_destroy)
    auto_s.protocol('WM_DELETE_WINDOW', quit_destroy)
    auto_s.mainloop()
